...
### Общая информация:
В этом проекте представлено решение задачи с временной сложностью O(|S|<sup>3</sup>) и O(2<sup>|S|</sup>*|S|), где **|S|** — длина исходной строки **S**. Чтобы собрать проект в Microsoft Visual Studio, клонируйте репозиторий, создайте папку `build`, откройте в ней командную строку и введите `cmake ..` (CMake должен быть установлен).
